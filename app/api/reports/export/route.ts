  ...
  try {
    ...
    if (!user || (user.role !== 'ADMIN' && user.role !== 'SUPERVISOR')) {
      return NextResponse.json(
        { error: 'Unauthorized. Only admins and supervisors can export reports.' },
        { status: 403 }
      )
    }
    ...
    if (type === 'leave-requests') {
      ...
      data = leaveRequests.map(request => ({
        ...
        'Days Requested': Math.ceil((request.endDate.getTime() - request.startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1
      }))
