 ...
 */
export function handleClerkError(error: any): { message: string; status: number } {
  ...
  if (error?.errors?.[0]) {
    ...
    switch (clerkError.code) {
      ...
      case 'form_param_format_invalid':
        return {
          message: 'Invalid format for one of the fields. Please check your input.',
          status: 400
        }
      ...
      default:
        return {
          message: clerkError.longMessage || clerkError.message || 'Authentication error occurred.',
          status: 400
        }
    }
  }
